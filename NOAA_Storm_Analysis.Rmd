...
#### + Types of events that are most harmful to population health:
    ...
    storm_impact %>%
      ...
      mutate("% of Injuries" = paste0(round(inj/ sum(inj), 2)*100, "%")) %>%
    ...
    ```{r}
        other_fat_sum <- sum(storm_impact$fat) - sum(filter(storm_impact, fat > quantile(fat,.9))$fat)
    storm_impact %>%
      select(EVTYPE, fat) %>%
      filter(fat > quantile(fat, .9)) %>%
      arrange(desc(fat)) %>%
      rbind(data.frame(EVTYPE = "OTHER", fat = other_fat_sum) ) %>%      
      mutate("% of Fatalities" = paste0(round(fat/ sum(fat), 2)*100, "%")) %>%
